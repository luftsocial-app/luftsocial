...
describe('MessageService', () => {
  ...
  const createMockMessage = (
    ...
    baseMessage.metadata = overrides.metadata || {
      reactions: {},
      editHistory: [],
    };
    if (!baseMessage.metadata.reactions) baseMessage.metadata.reactions = {};
    if (!baseMessage.metadata.editHistory)
      baseMessage.metadata.editHistory = [];
    ...
    baseMessage.addEditHistory = jest.fn().mockImplementation(function (
      this: MessageEntity,
      content: string,
    ) {
    ...
    baseMessage.addReaction = jest.fn().mockImplementation(function (
      this: MessageEntity,
      userId: string,
      emoji: string,
    ) {
    ...
    baseMessage.removeReaction = jest.fn().mockImplementation(function (
      this: MessageEntity,
      userId: string,
      emoji?: string,
    ) {
      ...
      this.reactions = this.reactions.filter(
        (r) => !(r.userId === userId && (emoji ? r.emoji === emoji : true)),
      );
    });
    baseMessage.markAsRead = jest.fn().mockImplementation(function (
      this: MessageEntity,
      userId: string,
    ) {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: { getTenantId: jest.fn().mockResolvedValue(mockTenantId) },
        },
        {
          provide: ContentSanitizer,
          useValue: {
            sanitize: jest.fn((content) => content),
            sanitizeRealtimeMessage: jest.fn((content) => ({
              isValid: true,
              sanitized: content,
            })),
  ...
  describe('createMessage', () => {
    ...
    it('should create a message successfully', async () => {
      ...
      messageRepositoryMock.create.mockImplementation((entity) =>
        createMockMessage(entity),
      );
      messageRepositoryMock.save.mockResolvedValue(
        createMockMessage(createDto),
      );
      ...
      expect(
        conversationServiceMock.updateLastMessageTimestamp,
      ).toHaveBeenCalledWith(mockConversationId);
      expect(tenantServiceMock.getTenantId).toHaveBeenCalledTimes(1);
      expect(contentSanititizerMock.sanitize).toHaveBeenCalledWith(
        'Test message',
      );
    });
    ...
    it('should create a message with a valid parentMessageId', async () => {
      ...
      messageRepositoryMock.save.mockResolvedValue(
        createMockMessage({ parentMessageId: 'parent-msg-id' }),
      );
      ...
      expect(messageRepositoryMock.findByIdAndTenant).toHaveBeenCalledWith(
        'parent-msg-id',
        mockTenantId,
      );
    ...
    it('should throw NotFoundException if parentMessageId not found', async () => {
      ...
      await expect(
        ...
        new NotFoundException(
          'Parent message not found or in a different conversation.',
        ),
      );
    });
  ...
  describe('updateMessageStatus', () => {
    it('should update message status successfully', async () => {
      ...
      expect(messageRepositoryMock.update).toHaveBeenCalledWith(
        { id: mockMessageId, tenantId: mockTenantId },
    ...
    it('should throw NotFoundException if message not found', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(null);
      await expect(
        service.updateMessageStatus(mockMessageId, MessageStatus.READ),
  ...
  describe('updateMessage', () => {
    ...
    it('should update message successfully', async () => {
      ...
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(
        originalMessage,
      );
      contentSanitizerMock.sanitize.mockReturnValue(updateDto.content);
      messageRepositoryMock.save.mockImplementation(
        async (msg: MessageEntity) => msg,
      );
      ...
      expect(contentSanitizerMock.sanitize).toHaveBeenCalledWith(
        updateDto.content,
      );
    ...
    it('should throw NotFoundException if message not found', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(null);
      await expect(
        service.updateMessage(mockMessageId, updateDto, mockUserId),
      ).rejects.toThrow(NotFoundException);
    });

    it('should throw ForbiddenException if user is not sender', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(
        createMockMessage({ senderId: 'other-user' }),
      );
      await expect(
        service.updateMessage(mockMessageId, updateDto, mockUserId),
      ).rejects.toThrow(ForbiddenException);
  ...
  describe('deleteMessage', () => {
    it('should delete message successfully if user is sender', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(
        createMockMessage({ senderId: mockUserId }),
      );
      await service.deleteMessage(mockMessageId, mockUserId);
      expect(messageRepositoryMock.markAsDeleted).toHaveBeenCalledWith(
        mockMessageId,
        mockUserId,
      );
    });
    ...
    it('should delete message successfully if user is admin', async () => {
      ...
      expect(conversationServiceMock.isUserAdmin).toHaveBeenCalledWith(
        mockUserId,
        mockConversationId,
      );
      expect(messageRepositoryMock.markAsDeleted).toHaveBeenCalledWith(
  ...
  describe('getMessages', () => {
    ...
    it('should return messages with unreadCount if query.userId is provided', async () => {
      ...
      expect(messageRepositoryMock.getUnreadCount).toHaveBeenCalledWith(
        mockConversationId,
        mockUserId,
        mockTenantId,
      );
    ...
    it('should throw ForbiddenException if validateAccess fails', async () => {
      conversationServiceMock.validateAccess.mockResolvedValue(false);
      await expect(
        service.getMessages(mockConversationId, query),
      ).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('getMessageHistory', () => {
    it('should retrieve message history', async () => {
      ...
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(
        createMockMessage({
          metadata: { reactions: {}, editHistory: history },
        }),
      );
    ...
    it('should return empty array for no history', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(
        createMockMessage({ metadata: { reactions: {}, editHistory: [] } }),
      );
      const result = await service.getMessageHistory(mockMessageId);
    ...
    it('should return empty array if metadata or editHistory is null/undefined', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(
        createMockMessage({ metadata: null }),
      );
      expect(await service.getMessageHistory(mockMessageId)).toEqual([]);

      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(
        createMockMessage({ metadata: { editHistory: undefined } as any }),
      );
      expect(await service.getMessageHistory(mockMessageId)).toEqual([]);
    });
    ...
    it('should throw NotFoundException if message not found', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(null);
      await expect(service.getMessageHistory(mockMessageId)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('addReaction', () => {
    ...
    it('should throw NotFoundException if message not found', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(null);
      await expect(
        service.addReaction(mockMessageId, mockUserId, '👍'),
      ).rejects.toThrow(NotFoundException);
    });
  ...
  describe('removeReaction', () => {
    it('should remove a reaction', async () => {
      const localMsg = createMockMessage({
        reactions: [{ userId: mockUserId, emoji: '👍' } as any],
      });
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(localMsg);
      messageRepositoryMock.save.mockImplementation(async (m) => m);
    ...
    it('should throw NotFoundException if message not found', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(null);
      await expect(
        service.removeReaction(mockMessageId, mockUserId, '👍'),
      ).rejects.toThrow(NotFoundException);
    });
  ...
  describe('getAttachments', () => {
    it('should return attachments', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(mockMessage);
      attachmentRepositoryMock.findByMessageId.mockResolvedValue([
    ...
    it('should throw NotFoundException if message not found', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(null);
      await expect(service.getAttachments(mockMessageId)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('getThreadReplies', () => {
    ...
    it('should return thread replies', async () => {
      ...
      expect(messageRepositoryMock.findOne).toHaveBeenCalledWith({
        where: { id: parentId, tenantId: mockTenantId },
      });
      expect(messageRepositoryMock.findThreadReplies).toHaveBeenCalledWith(
        parentId,
        mockTenantId,
        query,
      );
    ...
    it('should throw NotFoundException if parent message not found', async () => {
      ...
      await expect(service.getThreadReplies(parentId, query)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('findMessageById', () => {
    ...
    it('should throw NotFoundException if message not found', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(null);
      await expect(service.findMessageById(mockMessageId)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('markMessageAsRead', () => {
    ...
    it('should throw NotFoundException if message not found', async () => {
      messageRepositoryMock.findByIdAndTenant.mockResolvedValue(null);
      await expect(
        service.markMessageAsRead(mockMessageId, mockUserId),
      ).rejects.toThrow(NotFoundException);
    });
  });
