...
import { CreateConversationDto, AddParticipantsDto, UpdateConversationSettingsDto } from '../dto/conversation.dto';
...
export class ConversationController {
  ...
  async createOrGetDirectChat(
    ...
    return await this.conversationService.createOrGetDirectChat(user.id, otherUserId);
  }
  ...
  @ApiResponse({ status: 409, description: 'Cannot add participants to direct chat' })
  ...
  @ApiResponse({ status: 403, description: 'Insufficient permissions to update settings' })
  @Patch(':id/settings')
