  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit
{
  ...
  async handleConnection(client: SocketWithUser) {
    try {
      ...
      if (this.clientsPerUser.get(user.id).size > this.MAX_CLIENTS_PER_USER) {
        this.logger.warn(
          `User ${user.id} has too many connections (${this.clientsPerUser.get(user.id).size})`,
        );
  ...
  private handleError(client: SocketWithUser, errorEvent: ErrorEvent): void {
    this.logger.warn(
      `Error for client ${client.id}: ${errorEvent.code} - ${errorEvent.message}`,
    );
  ...
  async handleMessage(client: SocketWithUser, payload: MessageEventPayload) {
    try {
      ...
      this.logger.error(
        `Error in handleMessage: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async handleLeaveConversation(
    ...
    try {
      ...
      this.logger.error(
        `Error in handleLeaveConversation: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async handleStopTyping(client: SocketWithUser, payload: TypingEventPayload) {
    try {
      ...
      this.logger.error(
        `Error in handleStopTyping: ${error.message}`,
        error.stack,
      );
      return { success: false };
    }
  ...
  async handleMarkAsRead(client: SocketWithUser, payload: ReadReceiptPayload) {
    try {
      ...
      this.logger.error(
        `Error in handleMarkAsRead: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async handleMessageUpdated(
    ...
    try {
      ...
      this.logger.error(
        `Error in handleMessageUpdated: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async handleMessageDeleted(
    ...
    try {
      ...
      this.logger.error(
        `Error in handleMessageDeleted: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async handleReactionAdded(client: SocketWithUser, payload: ReactionPayload) {
    try {
      ...
      this.logger.error(
        `Error in handleReactionAdded: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async handleReactionRemoved(
    ...
    try {
      ...
      this.logger.error(
        `Error in handleReactionRemoved: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async handleParticipantAdded(
    ...
    try {
      ...
      try {
        const conversation =
          await this.conversationService.addParticipantsToGroup(
            payload.conversationId,
            payload.participantIds,
            user.id,
          );
      ...
      this.logger.error(
        `Error in handleParticipantAdded: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async handleParticipantRemoved(
    ...
    try {
      ...
      this.logger.error(
        `Error in handleParticipantRemoved: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
