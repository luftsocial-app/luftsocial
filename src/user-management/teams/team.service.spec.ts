...
describe('TeamService', () => {
  ...
  describe('createTeam', () => {
    ...
    it('should create a team successfully', async () => {
      ...
      expect(userRepository.findOne).toHaveBeenCalledWith({
        where: { id: mockCreatorId, activeTenantId: mockTenantId },
      });
    ...
    it('should throw NotFoundException if tenant not found', async () => {
      ...
      await expect(
        service.createTeam(createTeamDto, 'wrong-tenant-id', mockCreatorId),
    ...
    it('should throw NotFoundException if creator not found in tenant', async () => {
      ...
      await expect(
        service.createTeam(createTeamDto, mockTenantId, 'wrong-creator-id'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException on repository save error', async () => {
      ...
      await expect(
        service.createTeam(createTeamDto, mockTenantId, mockCreatorId),
  ...
  describe('getTeamById', () => {
    ...
    it('should throw NotFoundException if team not found', async () => {
      ...
      await expect(
        service.getTeamById('wrong-team-id', mockTenantId),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('getTeamsInTenant', () => {
    ...
    beforeEach(() => {
      mockTenant = { id: mockTenantId, name: 'Test Tenant' } as Tenant;
      mockTeams = [
        { id: mockTeamId, name: 'Team 1', tenant: mockTenant } as Team,
      ];
      jest.spyOn(tenantRepository, 'findOne').mockResolvedValue(mockTenant);
    ...
    it('should throw NotFoundException if tenant not found', async () => {
      jest.spyOn(tenantRepository, 'findOne').mockResolvedValue(null);
      await expect(service.getTeamsInTenant('wrong-tenant-id')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('updateTeam', () => {
    ...
    it('should update a team successfully', async () => {
      ...
      expect(teamRepository.findOne).toHaveBeenCalledWith({
        // from getTeamById call
        where: { id: mockTeamId, tenant: { id: mockTenantId } },
        relations: ['users', 'tenant', 'createdBy'],
      });
      expect(teamRepository.save).toHaveBeenCalledWith(
        expect.objectContaining(updateTeamDto),
      );
    });
    ...
    it('should throw NotFoundException if team to update is not found', async () => {
      jest.spyOn(teamRepository, 'findOne').mockResolvedValue(null);
      await expect(
        service.updateTeam('wrong-team-id', updateTeamDto, mockTenantId),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException on save error during update', async () => {
      jest
        ...
        .mockRejectedValue(new Error('DB error'));
      await expect(
        service.updateTeam(mockTeamId, updateTeamDto, mockTenantId),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('deleteTeam', () => {
    ...
    it('should throw NotFoundException if team to delete is not found', async () => {
      jest.spyOn(teamRepository, 'findOne').mockResolvedValue(null);
      await expect(
        service.deleteTeam('wrong-team-id', mockTenantId),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException on remove error during delete', async () => {
      jest
        ...
        .mockRejectedValue(new Error('DB error'));
      await expect(
        service.deleteTeam(mockTeamId, mockTenantId),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('addMemberToTeam', () => {
    ...
    beforeEach(() => {
      ...
      jest
        .spyOn(teamRepository, 'save')
        .mockImplementation(async (team: Team) => team); // Return the modified team
    });

    it('should add a member to a team successfully', async () => {
      const result = await service.addMemberToTeam(
        mockTeamId,
        mockUserId,
        mockTenantId,
      );
    ...
    it('should throw NotFoundException if team not found', async () => {
      jest.spyOn(teamRepository, 'findOne').mockResolvedValue(null);
      await expect(
        service.addMemberToTeam('wrong-team-id', mockUserId, mockTenantId),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw NotFoundException if user to add not found', async () => {
      jest.spyOn(userRepository, 'findOne').mockResolvedValue(null);
      await expect(
        service.addMemberToTeam(mockTeamId, 'wrong-user-id', mockTenantId),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it("should throw ForbiddenException if user does not belong to team's tenant", async () => {
      ...
      jest.spyOn(userRepository, 'findOne').mockResolvedValue(mockUserToAdd);
      await expect(
        service.addMemberToTeam(mockTeamId, mockUserId, mockTenantId),
  ...
  describe('removeMemberFromTeam', () => {
    ...
    beforeEach(() => {
      ...
      jest
        ...
        .mockResolvedValue(mockTeamWithMember); // For getTeamById
      jest
        .spyOn(teamRepository, 'save')
        .mockImplementation(async (team: Team) => team);
    });

    it('should remove a member from a team successfully', async () => {
      const result = await service.removeMemberFromTeam(
        mockTeamId,
        mockUserId,
        mockTenantId,
      );
    ...
    it('should throw NotFoundException if team not found', async () => {
      jest.spyOn(teamRepository, 'findOne').mockResolvedValue(null);
      await expect(
        service.removeMemberFromTeam('wrong-team-id', mockUserId, mockTenantId),
      ).rejects.toThrow(NotFoundException);
    });

    it('should throw NotFoundException if user to remove is not in the team', async () => {
      await expect(
        service.removeMemberFromTeam(
          mockTeamId,
    ...
    it('should throw BadRequestException on save error', async () => {
      jest
        ...
        .mockRejectedValue(new Error('DB error'));
      await expect(
        service.removeMemberFromTeam(mockTeamId, mockUserId, mockTenantId),
      ).rejects.toThrow(BadRequestException);
    });
  });
});
