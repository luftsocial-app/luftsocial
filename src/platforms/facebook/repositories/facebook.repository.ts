...
export class FacebookRepository {
    ...
    async createAccount(data: Partial<FacebookAccount>): Promise<FacebookAccount> {
        const account = this.accountRepo.create(data);
        return this.accountRepo.save(account);
    }

    async updateAccount(id: string, data: Partial<FacebookAccount>): Promise<FacebookAccount> {
    ...
    async getAccountById(id: string, relations: string[] = []): Promise<FacebookAccount> {
    ...
    async getPageById(id: string, relations: string[] = []): Promise<FacebookPage> {
    ...
    async getPostById(id: string, relations: string[] = []): Promise<FacebookPost> {
    ...
    // Unused
    async getRecentPostCount(pageId: string, timeframe: 'hour' | 'day'): Promise<number> {
    ...
    async createAuthState(userId: string): Promise<string> {
        ...
        // Update page token
        async updatePageToken(pageId: string, newToken: string): Promise<FacebookPage> {
    ...
    async upsertMetrics(data: {
        ...
        return this.entityManager.transaction(async transactionalEntityManager => {
            const existing = await transactionalEntityManager.findOne(FacebookPostMetric, {
                where: {
                    post: { id: data.postId },
            ...
            const newMetric = transactionalEntityManager.create(FacebookPostMetric, {
                post: { id: data.postId },
                ...data.metrics
            });
    ...
    async updatePost(postId: string, updateData: Partial<FacebookPost>): Promise<FacebookPost> {
        await this.postRepo.update(postId, updateData);
        return this.postRepo.findOne({ where: { id: postId }, relations: ['page'] });
     }
     
     async updatePage(pageId: string, updateData: Partial<FacebookPage>): Promise<FacebookPage> {
