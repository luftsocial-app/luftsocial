...
import { CreatePostDto, SchedulePagePostDto, SchedulePostDto, UpdatePageDto, UpdatePostDto } from './helpers/post.dto';
import { FacebookPost } from './entity/facebook-post.entity';
import { CommentResponse, PlatformService, TokenResponse } from '../platform-service.interface';
...
export class FacebookService implements PlatformService {
    ...
    async getComments(accountId: string, postId: string, pageToken?: string): Promise<CommentResponse> {
    ...
    async post(accountId: string, content: string, mediaUrls?: string[]): Promise<any> {
    ...
    async refreshLongLivedToken(token: string): Promise<any> {
     ...
     async refreshPageToken(pageId: string, userAccessToken: string): Promise<any> {
        const response = await axios.get(
    ...
    async createPagePost(pageId: string, createPostDto: CreatePostDto): Promise<FacebookPost> {
        ...
        const postData = await axios.post(
            `${this.baseUrl}/${this.apiVersion}/${page.pageId}/feed`,
            {
                ...
                ...(createPostDto.media && {
                    attached_media: await this.processMedia(page.accessToken, createPostDto.media.map(m => m.url))
                })
            },
            {
    ...
    async schedulePost(postId: string, scheduleDto: SchedulePostDto): Promise<FacebookPost> {
        ...
        const postData = await axios.post(
            `${this.baseUrl}/${this.apiVersion}/${page.pageId}/feed`,
            {
                ...
                published: false,
                scheduled_publish_time: new Date(scheduleDto.scheduledTime).getTime() / 1000,
                ...(scheduleDto.media && {
                    attached_media: await this.processMedia(page.accessToken, scheduleDto.media.map(m => m.url))
                })
            },
            {
                params: { access_token: page.accessToken }
    ...
    async schedulePagePost(scheduleDto: SchedulePagePostDto): Promise<FacebookPost> {
    ...
    async getPagePosts(pageId: string, limit: number = 10, cursor?: string): Promise<any> {
        const page = await this.facebookRepo.getPageById(pageId);
        const response = await axios.get(
            `${this.baseUrl}/${this.apiVersion}/${page.pageId}/feed`,
            {
    ...
    async editPost(postId: string, updateDto: UpdatePostDto): Promise<FacebookPost> {
        const post = await this.facebookRepo.getPostById(postId, ['page']);
        if (!post) throw new NotFoundException('Post not found');
        ...
        try {
            await axios.post(
                `${this.baseUrl}/${this.apiVersion}/${post.postId}`,
                {
                    ...
                    ...(updateDto.media && {
                        attached_media: await this.processMedia(post.page.accessToken, updateDto.media.map(m => m.url))
                    })
                },
                {
     ...
     async editPage(pageId: string, updateDto: UpdatePageDto): Promise<FacebookPage> {
        const page = await this.facebookRepo.getPageById(pageId);
        if (!page) throw new NotFoundException('Page not found');
    ...
    private async processMedia(accessToken: string, mediaUrls: string[]): Promise<any[]> {
