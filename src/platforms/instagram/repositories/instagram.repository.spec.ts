...
describe('InstagramRepository', () => {
  ...
  beforeEach(async () => {
    ...
    rateLimitRepositoryMock = module.get(
  ...
  describe('createAccount', () => {
    ...
    const savedInstagramAccount = {
      id: 'ig-acc-db-new-1',
      tenantId: inputData.tenantId,
      instagramAccountId: inputData.instagramId,
    ...
    it('should create SocialAccount and InstagramAccount in a transaction and return InstagramAccount', async () => {
      ...
      expect(socialAccountRepositoryMock.create).toHaveBeenCalledWith(
    ...
    it('should propagate error if transaction fails', async () => {
      ...
      await expect(repository.createAccount(inputData)).rejects.toThrow(
        dbError,
      );
    });
    ...
    it('should propagate error if socialAccountRepo.create fails', async () => {
      ...
      socialAccountRepositoryMock.create.mockImplementation(() => {
        throw createError;
      });
      entityManagerMock.transaction.mockImplementation(async (cb) =>
    ...
    it('should propagate error if transactionManager.save for SocialAccount fails', async () => {
      ...
      const mockTransactionManager = {
        save: jest.fn().mockRejectedValueOnce(saveSocialError),
      } as unknown as EntityManager;
      entityManagerMock.transaction.mockImplementation(async (cb) =>
        cb(mockTransactionManager),
      );
  ...
  describe('getAccountByUserId', () => {
    const mockSocialAcc = {
      ...
      platform: SocialPlatform.INSTAGRAM,
      tenantId: mockTenantId,
      platformUserId: 'ig-platform-id-1',
    } as SocialAccount;
    const mockIgAccount = {
      id: 'ig-account-db-id-1',
      instagramAccountId: 'ig-platform-id-1',
      username: 'igUser',
      socialAccount: mockSocialAcc,
      tenantId: mockTenantId,
    } as InstagramAccount;
    ...
    it('should return InstagramAccount if found via SocialAccount', async () => {
      ...
      expect(accountRepositoryMock.findOne).toHaveBeenCalledWith({
        where: {
          socialAccount: { id: mockSocialAcc.id },
          tenantId: mockTenantId,
        },
    ...
    it('should return null and log warning if SocialAccount not found', async () => {
      ...
      expect(loggerMock.warn).toHaveBeenCalledWith(
        `No Instagram social account found for Clerk User ID: ${mockClerkUserId} in tenant: ${mockTenantId}`,
      );
    ...
    it('should return null and log error if InstagramAccount not found (data inconsistency)', async () => {
      ...
      expect(loggerMock.error).toHaveBeenCalledWith(
        `Data inconsistency: Instagram SocialAccount ${mockSocialAcc.id} found but no corresponding InstagramAccount for Clerk User ID: ${mockClerkUserId} in tenant: ${mockTenantId}`,
      );
    });

    it('should propagate error if tenantService.getTenantId fails', async () => {
      const error = new Error('Tenant service error');
      tenantServiceMock.getTenantId.mockImplementation(() => {
        throw error;
      });
      await expect(
        repository.getAccountByUserId(mockClerkUserId),
  ...
  describe('getTopPerformingMedia', () => {
    it('should get top performing media', async () => {
      ...
      mediaRepositoryMock.createQueryBuilder.mockReturnValue(
        queryBuilder as any,
      );
  ...
  describe('getActiveAccounts', () => {
    it('should get active accounts', async () => {
      ...
      expect(accountRepositoryMock.find).toHaveBeenCalledWith(
        expect.objectContaining({
          where: {
            tenantId: mockTenantId,
            socialAccount: { tokenExpiresAt: MoreThan(expect.any(Date)) },
          },
          relations: ['socialAccount'],
        }),
      );
    ...
    it('should get accounts with expiring tokens', async () => {
      ...
      expect(accountRepositoryMock.find).toHaveBeenCalledWith(
        expect.objectContaining({
          where: {
            tenantId: mockTenantId,
            socialAccount: { tokenExpiresAt: LessThan(expect.any(Date)) },
          },
          relations: ['socialAccount'],
        }),
      );
  ...
  describe('updateAccountTokens', () => {
    it('should update account tokens', async () => {
      ...
      accountRepositoryMock.findOne.mockResolvedValue({
        id: 'acc-1',
      ...
      jest
        ...
        .mockResolvedValue({} as InstagramAccount); // Mock internal call
      await repository.updateAccountTokens('acc-1', {
        accessToken: 'new',
        expiresAt: new Date(),
      });
    ...
    it('should throw NotFoundException if account not found', async () => {
      accountRepositoryMock.findOne.mockResolvedValue(null);
      await expect(
        repository.updateAccountTokens('acc-1', {
          accessToken: 'new',
          expiresAt: new Date(),
        }),
  ...
  describe('deleteAccount', () => {
    ...
    it('should throw NotFoundException if account not found for deletion', async () => {
      accountRepositoryMock.findOne.mockResolvedValue(null);
      await expect(repository.deleteAccount('acc-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
