...
describe('InstagramService', () => {
  ...
  const mockInstagramAccountEntity = {
    ...
    metadata: {
      ...
      instagramAccounts: [
        // This structure might be from an older version or specific use case
        {
          id: mockIgPlatformUserId,
          username: 'test_instagram',
          pageId: 'test-page-id',
        },
      ],
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ConfigService, useValue: mockConfigServiceProvider },
        {
          ...
          useValue: mockMediaStorageServiceProvider,
        },
    ...
    instagramRepoMock.getAccountByUserId.mockResolvedValue(
      mockInstagramAccountEntity,
    );
    ...
    mediaStorageServiceMock.uploadPostMedia.mockResolvedValue([
      { url: 'http://example.com/uploaded.jpg' } as any,
    ]);
    mediaStorageServiceMock.uploadMediaFromUrl.mockResolvedValue({
      url: 'http://example.com/uploaded_from_url.jpg',
    } as any);
  });
  ...
  describe('getAccountsByUserId', () => {
    it('should return Instagram account if found by repository', async () => {
      ...
      expect(instagramRepoMock.getAccountByUserId).toHaveBeenCalledWith(
        mockUserId,
      );
      ...
      expect(result).toEqual([
        {
          ...
          profilePictureUrl:
            mockInstagramAccountEntity.metadata?.profilePictureUrl,
          platform: 'INSTAGRAM',
          status: 'ACTIVE',
          tokenExpiresAt:
            mockInstagramAccountEntity.socialAccount.tokenExpiresAt,
        },
      ]);
    });
  ...
  describe('getAccountInsights', () => {
    it('should retrieve account insights if account exists', async () => {
      ...
      expect(instagramRepoMock.getAccountByUserId).toHaveBeenCalledWith(
        mockAccountId,
      ); // Service uses accountId (db id) to fetch from repo
      expect(mockedAxios.get).toHaveBeenCalledWith(
        expect.stringContaining(`/${mockIgPlatformUserId}/insights`),
        expect.any(Object),
      );
  ...
  describe('createStory', () => {
    it('should create an Instagram story if account exists', async () => {
      ...
      expect(result).toEqual({
        platformPostId: mockMediaContainerId,
  ...
  describe('revokeAccess', () => {
    it('should revoke access and delete account if account exists', async () => {
      ...
      expect(mockedAxios.post).toHaveBeenCalledWith(
        expect.stringContaining(`/${mockIgPlatformUserId}/permissions`),
        null,
        expect.any(Object),
      );
      expect(instagramRepoMock.deleteAccount).toHaveBeenCalledWith(
        mockAccountId,
      );
    });
  ...
  describe('withRateLimit', () => {
    it('should execute callback if rate limit not exceeded', async () => {
      ...
      expect(instagramRepoMock.checkRateLimit).toHaveBeenCalledWith(
        mockAccountId,
        action,
      );
      expect(mockCallback).toHaveBeenCalled();
      expect(instagramRepoMock.recordRateLimitUsage).toHaveBeenCalledWith(
        mockAccountId,
        action,
      );
    ...
    it('should throw HttpException if rate limit exceeded', async () => {
      ...
      await expect(
        service.withRateLimit(mockAccountId, action, mockCallback),
      ).rejects.toThrow(
  ...
  describe('post (example of method using accountId as DB ID)', () => {
    it('should make a post if account exists', async () => {
      ...
      mockedAxios.post.mockImplementation((url) => {
        if (url.includes(`${mockIgPlatformUserId}/media`))
          return Promise.resolve({ data: { id: 'container_id' } });
        if (url.includes(`${mockIgPlatformUserId}/media_publish`))
          return Promise.resolve({ data: { id: 'published_media_id' } });
    ...
    it('should throw NotFoundException if account not found for post', async () => {
      ...
      await expect(
        service.post(postData.accountId, postData.mediaItems, postData.caption),
      ).rejects.toThrow(
