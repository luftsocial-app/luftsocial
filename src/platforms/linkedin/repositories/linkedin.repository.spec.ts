...
describe('LinkedInRepository', () => {
  ...
  const mockEntityManagerFactory = () => ({
    update: jest.fn(),
    save: jest.fn(),
    transaction: jest.fn((cb) => cb(mockEntityManagerFactory())),
    remove: jest.fn(),
    delete: jest.fn(),
  ...
  describe('createAccount', () => {
    ...
    const savedLinkedInAccount = {
      id: 'li-acc-db-new-1',
      tenantId: inputData.tenantId,
      linkedinUserId: inputData.linkedinUserId,
    ...
    it('should create SocialAccount and LinkedInAccount in a transaction', async () => {
      ...
      expect(socialAccountRepoMock.create).toHaveBeenCalledWith(
  ...
  describe('getAccountByClerkUserId', () => {
    ...
    const mockLiAccount = {
      id: mockLinkedInAccountId,
      linkedinUserId: mockPlatformUserId,
    ...
    it('should return LinkedInAccount if found via SocialAccount for the given Clerk User ID', async () => {
      ...
      expect(loggerMock.debug).toHaveBeenCalledWith(
        `Attempting to find LinkedIn account for Clerk User ID: ${mockClerkUserId} in tenant: ${mockTenantId}`,
      );
    ...
    it('should return null and log warning if SocialAccount not found for Clerk User ID', async () => {
      ...
      expect(loggerMock.warn).toHaveBeenCalledWith(
        `No LinkedIn social account found for Clerk User ID: ${mockClerkUserId} in tenant: ${mockTenantId}`,
      );
    ...
    it('should return null and log error if LinkedInAccount not found for an existing SocialAccount (data inconsistency)', async () => {
      ...
      expect(loggerMock.error).toHaveBeenCalledWith(
        `Data inconsistency: LinkedIn SocialAccount ${mockSocialAcc.id} found but no corresponding LinkedInAccount for Clerk User ID: ${mockClerkUserId} in tenant: ${mockTenantId}`,
      );
    });
  });
  ...
  describe('getById', () => {
    const mockLiAccountForGetById = {
      id: mockLinkedInAccountId,
      tenantId: mockTenantId,
      linkedinUserId: 'some-li-user-id',
      socialAccount: { id: 'sa1' } as SocialAccount,
      organizations: [],
    } as LinkedInAccount;
  ...
  describe('createAuthState', () => {
    it('should create auth state', async () => {
      entityManagerMock.save.mockResolvedValue({
        state: 'mock-state-value',
      } as any); // for auth_states table
  ...
  describe('updateAccountTokens', () => {
    it('should update tokens', async () => {
      accountRepoMock.findOne.mockResolvedValue({
        id: 'acc-id',
        socialAccount: { id: 'sa-id' },
      } as LinkedInAccount);
      ...
      await repository.updateAccountTokens('acc-id', {
        accessToken: 'new',
        expiresAt: new Date(),
      });
