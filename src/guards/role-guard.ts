...
export class RoleGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!userId) {
      this.logger.warn('No userId found on request.auth.userId. Ensure ClerkAuthGuard runs before RoleGuard.');
    ...
    try {
      ...
      if (!user || !user.roles || user.roles.length === 0) {
        this.logger.warn({ userId, requiredRoles }, 'User not found by UserService, or has no roles.');
      ...
      if (!hasRequiredRole) {
        this.logger.info({ userId, userRoles: user.roles.map(r => r.name), requiredRoles }, 'User does not have any of the required roles.');
      }
      
      return hasRequiredRole;

    } catch (error) {
      this.logger.error({ error, userId, requiredRoles }, 'Error in RoleGuard while fetching user or checking roles');
